...
public class FilterFramework extends Thread {
    ...
    private YFilterFramework YInputFilter;
     ...
     *
     ****************************************************************************/
    void ConnectY(YFilter Filter) {
        try {
            // Connect this filter's input to the upstream pipe's output stream

            System.out.println("\n" + this.getName() + "  Y-connected to " + Filter.getName());
    ...
    void Connect(FilterFramework Filter) {
        try {
            ...
            System.out.println("\n" + this.getName() + "  connected to " + Filter.getName());
            InputReadPort.connect(Filter.OutputWritePort);
            InputFilter = Filter;
    ...
    byte ReadFilterInputPort() throws EndOfStreamException {
        ...
        try {
            ...
            System.out.println("\n" + this.getName() + " " + this.getClass() + " Error in read port wait loop::" + Error);
            System.exit(0);
        } // catch
